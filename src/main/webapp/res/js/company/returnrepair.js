...
function bylikeReturnRepairDoucuments() {
	$("#checkbtn").on("click",function() {
		...
		firstLoadData("ReturnRepair/bylikeReturnRepairDoucuments.do?" + mySelectForm, 1);
...
function loadPageDatas(index) {
	...
	firstLoadData($("#path").val() + "/ReturnRepair/bylikeReturnRepairDoucuments.do", index);
}
// 分页查询以后前台页面打印
...
function addReturnRepairDoucuments() {
	$("#addInputstorage").click(function() {
		...
		$.ajax({
			url : '/ReturnRepair/addReturnRepairDoucuments.do',
			datatype : 'json',
			type : 'post',
			data : {
				addTime : date,
...
function delReturnRepairDoucuments() {
	$(".delReturnRepairDoucuments").bind("click", function() {
		...
		if (confirm('确定要删除此条数据吗?')) {
			$.ajax({
				url : '/ReturnRepair/deleteReturnRepairDoucuments.do',
				dataType : 'json',
				type : 'post',
				data : {
					sid : id,
				...
				success : function(data) {
					firstLoadData("ReturnRepair/bylikeReturnRepairDoucuments.do?" + mySelectForm,$(".laypage_curr").text());
				}
			});
		} else {
			return false;
...
function getupp() {
	$(".selectByPrimaryKey").bind("click", function() {
		...
		$.ajax({
			url : '/ReturnRepair/selectByPrimaryKey.do',
			dataType : 'json',
			type : 'post',
			data : {
				sid : id
...
function updateReturnRepairDoucuments() {
	$("#uppInputstorage").click(function() {
		...
		$.ajax({
			url : '/ReturnRepair/updateReturnRepairDoucuments.do',
			dataType : 'json',
			type : 'post',
			data : {
				sid : id,			
