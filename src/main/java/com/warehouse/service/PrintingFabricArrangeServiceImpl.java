...
import com.warehouse.dao.PrintingFabricArrangeMapper;
import com.warehouse.entity.PrintingFabricArrange;
import com.warehouse.util.PageUtil;

@Service
public class PrintingFabricArrangeServiceImpl implements PrintingFabricArrangeService {

	@Resource
	private PrintingFabricArrangeMapper mapper;
	...
	public Map<String, Object> getDataByPage(PageUtil pageUtil, PrintingFabricArrange ready) {
		...
		List<PrintingFabricArrange> dataList = this.mapper
				.getDataByPage(new PrintingFabricArrange(pageUtil.getReqult()[0], pageUtil.getReqult()[1], ready));
	...
	public void toExcle(HttpServletRequest request, HttpServletResponse response, String ids) {
		...
		// 得到id的集合
		String idList = "(" + ids + ")";
		List<PrintingFabricArrange> toExcleList = this.mapper.toExcle(idList);
		for (int i = 0; i < toExcleList.size(); i++) {
			row = sheet.createRow((int) i + 1);// 新增行
			PrintingFabricArrange ready = toExcleList.get(i);
