...
function loadPageDatas(index) {
	firstLoadData("/printMarkOrder/printMarkOrderBypage.do", index);
}
// 分页查询以后前台页面打印
...
function getProces(mydata) {
	...
	var revoked="<button class='btn btn-danger btn-sm revokOrder'  data-toggle='modal'  href='javascript:;'>撤 销</batton>";
	$("#tbody").append(
				"<tr>"
				...
				+ "</td>"
				+ "<td style='text-align: center;'>"
				+ "<a target='_blank' href='distributeDetailedOrder.html?"
				+ op.orderId+"'>领料详单</a>"
				+ "</td>"
				+ "<td style='text-align: center;'>"
...
function addPrint(){
	$("#addProce").on("click",function(){
		var printingFactory=$("#printingFactory").val();
		var orderRemark=$("#orderRemark").val();
		$.ajax({
			url:'printMarkOrder/addPrintMarkOrder.do',
			dataType:'json',
			type:'post',
			data:{
			...
			success:function(data){
				...
				firstLoadData("printMarkOrder/printMarkOrderBypage.do", 1);
			},error:function(){
				alert("添加失败系统错误！！");
			}
		});
...
function uppPrint(){
	$(".uppStatus").on("click",function(){
		...
		window.open("piecesInformation.html?"+ id);
	});
}
//打单
function printOrder(){
...
//		date=mydate.getFullYear()+"-"+(mydate.getMonth()+1)+"-"+mydate.getDate();
		$.ajax({
			url:'printMarkOrder/detailedOrders.do',
			dataType:'json',
			type:'post',
			data:{
				orderId:id
				...
				$.each(data,function(index, op) {
					$("#factory").append(op.printingFactory);
					return false;
				});
				//循环输出打印
				$.each(data,function(index, op) {
				...
				$("#prints").jqprint();
				firstLoadData("printMarkOrder/printMarkOrderBypage.do",$(".laypage_curr").text());
			},error:function(){
				alert("打单失败！系统错误！！");
			}
		});
...
function markOrder(){
	$(".markOrder").on("click",function(){
		...
		if (confirm('确定要发料吗?')) {
			$.ajax({
				url:'printMarkOrder/uppStatus.do',
				dataType:'json',
				type:'post',
				data:{
					orderId:id,
...
function uppblur(id, name) {
	$("#" + id).blur(function() {
		...
		data[name] = newtxt;
		$.ajax({
			url : 'printMarkOrder/dbuppPrintMarkOrder.do',
			dataType : 'json',
			type : 'post',
			data : data,
			success : function(data) {
...
function printingByLike(){
	$("#checkbtn").on("click",function(){
		var factory=$("#checkByFactory").val().trim();
		firstLoadData("/printMarkOrder/printMarkOrderBypage.do?printingFactory="+factory, 1);
	});
}