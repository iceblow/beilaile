...
function loadPageDatas(index) {
	...
	// 处理后的数据
	firstLoadData($("#path").val() + "/fabricOutflowReady/getDataByPage.do",
			index);
	...
	$("#mySelectProduct").click(
			function() {
				...
				firstLoadData($("#path").val()
						+ "/fabricOutflowReady/getDataByPage.do?"
						+ mySelectForm, 1);
				//$(".col-md-2>input").val("");
			});
}
...
function myblur(id) {
	$("#" + id).blur(
			function() {
				...
				$.ajax({
					url : $("#path").val()
							+ '/fabricOutflowReady/dbUpdateData.do',
					type : 'POST',
					dataType : 'json',
					data : myform,
					async : false,
...
function commitTime() {
	var myform = $("#dbUpdateForm").serialize();
	$.ajax({
		url : $("#path").val() + '/fabricOutflowReady/dbUpdateData.do',
		type : 'POST',
		dataType : 'json',
		data : myform,
		async : false,
...
function completeReady() {
	$(".completeReady").click(
			function() {
				...
				$.post($("#path").val()
						+ '/fabricOutflowReady/completeReady.do', {
					id : id
				}, function(data) {
					if (data == 1) {
						alert('配备完成');
...
function completeSuccess() {
	$(".completeSuccess").click(
			function() {
				var id = $(this).parent().parent().find('td:eq(1)').text();
				$.post($("#path").val()
						+ '/fabricOutflowReady/completeSuccess.do', {
					id : id
				}, function(data) {
					if (data == 1) {
						alert('发放完成');
	...
	$("#toExcle").on("click", function() {
		...
		if (ids == "") {
			...
			if (confirm('确定导出这些数据吗?')) {
				window.open('/fabricOutflowReady/toExcle.do?ids=' + ids);
			} else {
				return false;
			}
		}
