...
import com.warehouse.dao.PutInStorageReportDAO;
import com.warehouse.entity.MyStorage;

@Service
public class PutInStorageReportServiceImpl implements PutInStorageReportService {

	@Resource
	private PutInStorageReportDAO putInStorageReportDAO;
	...
	public Map<String, Object> getDate(int con, int cdate, String form) {
		...
		// 得到日期的查询结果
		List<String> dao = this.putInStorageReportDAO.getDate(form, cdate);
		// 将日期顺序变为升序
		...
		// 2.获取图例(例如某个厂家或某个员工)
		List<String> factory = this.putInStorageReportDAO.getFactory(form);
		String myFactory = "";
		...
		// 3.拼接出图标数据
		List<MyStorage> allList = this.putInStorageReportDAO.getAll(form, date.get(0));
				...
				for (int j = 0; j < dates.length; j++) {
					for (int k = allList.size() - 1; k >= 0; k--) {
						...
						if (allList.get(k).getFactory().equals(factorys[i])
							...
							t = (allList.get(k).getTotal_delivery()==null?0:allList.get(k).getTotal_delivery());
						}
						totals[j] = t;
						if (allList.get(k).getFactory().equals(factorys[i])
								&& allList.get(k).getSdate().equals(dates[j])) {
